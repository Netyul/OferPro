...
$lembrarAdmin = (isset($_COOKIE['lembrarAdmin']) && $_COOKIE['lembrarAdmin'] != '' ? $_COOKIE['lembrarAdmin'] : false);
 ...
 $urlpost =  $_SERVER['HTTP_HOST'].'/admin/lojista/';
 ...
 if(isset($_POST['enviar'])){
	 ...
	 elseif(empty($input['senha'])){
		 ...
		 $queryLogin = "SELECT * FROM admin WHERE login='".$input['login']."'";
		 ...
		 if($LinhasLogin <= 0){
			...
			$querySenha = "SELECT * FROM admin WHERE login ='".$input['login']."' AND senha = '".$input['senha']."'";
			...
			if($LinhasSenha <=0){
				...
				$queryStatus = "SELECT * FROM admin WHERE login= '".$input['login']."' AND senha = '".$input['senha']."' AND statos = 'on'";
				...
				if($LinhasStatus <=0){
					...
					$_SESSION['Admin'] = $adm['nome'];
					$_SESSION['AdmSenha']   = $adm['senha'];
					$_SESSION['level'] = $adm['level'];
					if($input['lembrar'] == true){
						setcookie('lembrarAdmin', base64_encode($adm['id']).'/'.base64_encode($adm['login']).'/'.base64_encode($input['senha']), time() + (60 * 60 * 24 * 30), '/');
					}
					else{
						setcookie('lembrarAdmin', '', time() - (60 * 60 * 24 * 30), '/');						
					}
...
<head>
<meta charset="utf-8">
<title>login lojista OferApp</title>
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
            	...
            	<form class="form-horizontal" role="form" action="http://<?php  echo $_SERVER['HTTP_HOST']; ?>/admin/login/" method="post">
                  <div class="form-group">
                    <label class="col-sm-3 control-label">Login: </label>
                    <div class="col-sm-8">
                      <input type="text" class="form-control" name="login" id="login" value="<?php  echo $login; ?>" required/>
                    </div>
                  </div>
                  <div class="form-group">
