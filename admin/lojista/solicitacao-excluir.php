...
$query_Recordset1 = sprintf("SELECT * FROM solicitacoes WHERE id = %s", GetSQLValueString($colname_Recordset1, "int"));
...
if ((isset($_GET['solicitacao'])) && ($_GET['solicitacao'] != "")) {
  ...
  $deleteGoTo = "solicitacao.php?id=" . $row_Recordset1['id_lojista'] . "&action=excluido";
...
mysql_select_db($database_dboferapp, $dboferapp);
$query_solicitacao  = sprintf("SELECT * FROM solicitacoes WHERE id = %s", GetSQLValueString($colname_solicitacao, "int"));
...
if ((isset($_GET['solicitacao2'])) && ($_GET['solicitacao2'] != "")) {
  ...
  $deleteGoTo = "solicitacoes-vendidas.php?id=" . $row_solicitacao['id_lojista'] . "&action=excluido";
  if (isset($_SERVER['QUERY_STRING'])) {
    $deleteGoTo .= (strpos($deleteGoTo, '?')) ? "&" : "?";
    $deleteGoTo .= $_SERVER['QUERY_STRING'];
  }
